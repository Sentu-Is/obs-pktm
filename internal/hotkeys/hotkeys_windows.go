...
func keyboardProc(nCode int, wParam, lParam uintptr) uintptr {
	if nCode >= 0 && activeListener != nil {
		...
		if activeListener.handleKey(wParam, info.VkCode) {
			return 1
		}
	}
...
func (l *Listener) handleKey(message uintptr, vkCode uint32) bool {
	if !l.isHandledKey(vkCode) {
		return false
	}
...
func (l *Listener) isHandledKey(vkCode uint32) bool {
	switch vkCode {
	case vkF9, vkF10, vkF12:
		return true
	default:
		return false
	}
